...
x_entries = entries.sort_by(&:x)
y_entries = entries.sort_by(&:y)

entries.reject! { |e| e.infinite?(x_entries, y_entries) }

entry_map = entries.to_h { |e| [e, 0] }
...
for x in x_range
  for y in y_range
    nearest = Coord.nearest(x, y, x_entries)
    if entry_map.key? nearest
      entry_map[nearest] += 1
    end
  end
end

puts entry_map.values.max
